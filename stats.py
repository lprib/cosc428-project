...
def do_stat_test(distance_power):
    ...
    def main_callback(frame, key, mouse_x, mouse_y):
        ...
        if succ:
            ...
            images, angle = do_hough_image(control_img, -1, 49, 144, 10, distance_power)
            cv.imshow("main", np.concatenate(images, axis=1))
            if key == ord(' '):
                angles.append(angle)
                print(len(angles))

    run_camera_loop(1280, 720, "data/camera_matrix_720.npy", "data/distortion_coeff_720.npy", "main", main_callback)
...
def analyse(distance_power, subplot):
    ...
    angles = np.array([float(x.strip()) for x in open(filename).readlines() if x != "None\n"])
